...
int BSP_algorithm::randNum(int min, int max)
{
    random_device rd;
    mt19937 gen(rd());
...
void BSP_algorithm::bsp(vector<vector<int>> &map, int x, int y, int W, int H, int max_room_size)
{
    ...
    else
    {
        ...
        if (firstRoom)
        {
            ...
            player_pos.first = room_center_x;
            player_pos.second = room_center_y;
            firstRoom = false;
        }
        else
        {
...
void BSP_algorithm::draw_floors_and_paths(vector<vector<int>> &map)
{
...
void BSP_algorithm::draw_walls(vector<vector<int>> &map)
{
...
void BSP_algorithm::draw_trees(vector<vector<int>> &map)
{
    for (int i = 0; i < map.size(); i++)
    {
        for (int j = 0; j < map[i].size(); j++)
...
void BSP_algorithm::draw_keys_and_chest(vector<vector<int>> &map)
{
    set<int> s;
    while (s.size() < 4)
    {
